...
| `js/attachments.js` | **Original-Dateien der Bons in IndexedDB** (DB `grocery-share`, Store `attachments`, Index auf `receiptId`; Eintrag `{id, receiptId, mediaType, base64, page, createdAt}`) — localStorage ist für Fotos zu klein. Gespeichert wird die aufbereitete Analyse-Version (JPEG max. 2000px, PDFs original) nach jeder erfolgreichen Analyse; Neu-Analysen hängen an (page zählt weiter). `receipt.attachmentCount` steuert die „Original ansehen"-Buttons (Review + Auswertung → Viewer-Modal, PDFs via Blob-URL im neuen Tab). Backup-Export/-Import nimmt alle Anhänge mit (`getAllAttachments`/`replaceAllAttachments`); Bon-Löschen und „Alles löschen" räumen mit auf. |
...
| `js/app.js` | Vue 3 app. One large `reactive` state, Screens: home / review / summary / month / chat / settings. Assistant-`actions` werden in `applyAssistantAction` angewandt (rename_person, add/rename/remove_category, set/remove_rule, set_default_category — Pfand-Kategorie ist löschgeschützt) und im Chat als Chips bestätigt. Spracheingabe via Web Speech API (`de-DE`). Exposes `window.__gs = { state, newDraft, ... }` on localhost for E2E tests. Preis-Inputs halten Rohtext (`priceInput`), Cents werden geparst (`toCents` versteht deutsche Kommas). `reopenReceipt` legt eine persistierte Sicherungskopie (`r.editBackup`: store/date/totalCents/notes/items — bewusst OHNE apiCost/attachmentCount) an; `discardChanges` („Verwerfen"-Button, IMMER sichtbar) stellt sie wieder her; ohne Kopie aber `wasFinal` → Bon wird unverändert wieder abgeschlossen; neuer Entwurf → Verwerfen löscht ihn (separater Löschen-Button nur bei `wasFinal`, sonst redundant). Speichern/`finalizeReceipt` löscht die Kopie. |
